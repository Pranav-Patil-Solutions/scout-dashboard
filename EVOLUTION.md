...
## 2026-07-31 — Import 500s: migration 0006 shipped, never applied to Turso (repeat of delta 18)

35. **The delta 18 rule ("migrate Turso before restarting/shipping") was violated again, and this time the failure was invisible for an extra step because the caller swallowed the real error.** `drizzle/0006_wide_human_robot.sql` (JOBDASH-010's `fit_grade`/`fit_assessment`/`graded_at`/`graded_resume_v`/`graded_facts_v` + index) existed on disk and in `schema.ts`, but was never run against the live Turso DB — every `/api/import` POST that reached the insert step threw `SQLite error: table scout_jobs has no column named fit_grade`. That never surfaced: `daily-run.sh`'s import step used `curl -f`, which discards the response body on any non-2xx status, so the log only ever said "import FAILED — dashboard did not respond," indistinguishable from an actual timeout. Diagnosing it took reproducing the exact insert directly against Turso via `@libsql/client` to get the real `LibsqlError`. → **Delta:** (a) `daily-run.sh` no longer uses `curl -f`; it captures the HTTP status and the response body (success or failure) so a real 500 shows its actual error text in the log, not a generic timeout-shaped message. (b) Re-confirming delta 18's rule: schema + migration ship together, and the migration runs against Turso in the SAME step as the code change — not "when convenient." (c) `scripts/migrate.ts` reads `process.env.TURSO_*` directly with no dotenv load, so running it needs `.env.local` explicitly sourced into the shell first (`set -a; ... ; set +a`) — bare `npm run db:migrate` silently falls back to the local `scoutdash.db` file and reports "up to date" with zero signal that it migrated the WRONG database. Rule: any error-swallowing flag (`curl -f`, `2>/dev/null`, bare try/catch) on a step that writes to a database is a debt — log the real failure body, always.
Gate: migration applied + verified via direct column query on Turso; `/api/import` re-run live: `{"ok":true,"imported":114,"updated":416,"total":530,"graded":13}`.

## 2026-07-31 — JOBDASH-011 "Update everything" button

36. **A pipeline that only runs from a terminal is a pipeline only one person can trigger, and only from one machine.** `daily-run.sh` (scrape → rank/grade → import) only ran via launchd or a manual shell invocation — no way to kick it off from the dashboard itself. → **Delta:** `lib/scout-refresh.ts` (`runFullScoutUpdate`, "use server") spawns the scraper's `.venv/bin/python3 main.py` with its `.env` merged into the child env (main.py has no dotenv of its own — daily-run.sh sources the file into the shell before invoking it, so a spawned child must do the same or Apify/SMTP creds go missing), then calls the existing `importScoutJobs()`. Mac-only via the same self-disable convention as `importScoutJobs()` (derives the scraper repo dir from `JOBSCRAPER_DB_PATH` rather than adding a second env var). Wired to a new "Update everything" button beside "Import from scout" on the Discover board, plus a curlable `POST /api/scout-refresh` mirroring the existing `/api/import` route shape. **Verification trap avoided:** a first smoke-test attempt ran the new server action directly via `tsx` outside Next's runtime and got a `server-only` throw ("cannot be imported from a Client Component") — a false failure of the test harness, not the code, since bare Node/tsx doesn't set the `react-server` condition `server-only` checks for. Rule: a "use server" / `server-only` module can only be exercised through the actual running Next server (a real HTTP call), never via a standalone script runner, however convenient — that's exactly why the mirrored API route exists, and it earned its keep on the first try. Gate: live end-to-end via `POST /api/scout-refresh`: `{"ok":true,"import":{"imported":37,"updated":530,"total":567,"graded":6}}`.

## 2026-08-04 — Berlin retarget: a bounded sweep must be ordered by what it is for

37. **The fit-grade sweep was bounded (25/import) but unordered, so it spent its whole budget on rows that could never become a To-apply card.** After the Berlin retarget imported 68 fresh jobs, `/api/import` reported `autoApplied: 0` with 29 rows sitting at status=new + emailed + score>=75. All 29 were UNGRADED, and JOBDASH-010's gate treats ungraded as not-apply-eligible in BOTH paths — `autoSyncEmailedJobs()` and the manual Apply click in Discover (`applyToScoutJob`, actions.ts:154). So the board could not receive a single job, by either route. Cause: `gradeStaleScoutJobs()` selected `where(ungraded or stale).limit(25)` with no ORDER BY — SQLite returned rows in rowid order, i.e. the oldest imports, while the ungraded backlog was 746 rows deep. The strong fits were never reached and never would be. -> **Delta:** the sweep now orders by `score DESC`. When a job queue is bounded well below its backlog, ordering is not a nicety — it decides whether the feature ever fires. Any `.limit()` over a backlog needs an explicit ORDER BY justified by what the consumer of those rows needs.
**Second finding, and the more valuable one: the F wave is the grader working, not failing.** Of the top 10 by keyword score, 9 graded F and 1 graded C. The verdicts are specific and correct — ALGOTEQUE's 95.2 "Automation Lead SME" is a GMP/OT engineering role (DCS/SCADA/PLC in biopharma), Grab is onsite Jakarta with no work rights, NetBet needs Malta relocation plus professional copywriting, inca requires German fluency against an A2 candidate. The title-level score is a recall pre-filter and nothing more; treat a 90+ keyword score as "worth an LLM call", never as evidence of fit. Do not "fix" a low pass rate by loosening the gate — the pass rate IS the finding.
Gate: tsc 0, vitest 154/154, `next build` green, 3312 rebuilt + restarted (delta 28), live sweep 10/10 graded 0 failures.
